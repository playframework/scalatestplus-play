...
object ConfiguredAppParallelSpec {
  var timesApplicationIsBuilt = new AtomicInteger(0)
}

class ConfiguredAppParallelSpec extends Suites(new ConfiguredAppParallelNestedSpec()) with GuiceOneAppPerSuite {
  import ConfiguredAppParallelSpec._
...
@DoNotDiscover
class ConfiguredAppParallelNestedSpec extends UnitSpec with ConfiguredApp with ParallelTestExecution {
  import ConfiguredAppParallelSpec._
  ...
  "The ConfiguredApp trait, when running in parallel" must {
