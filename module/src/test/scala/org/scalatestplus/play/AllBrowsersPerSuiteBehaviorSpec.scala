...
class AllBrowsersPerSuiteBehaviorSpec extends WordSpec {
  ...
  "The AllBrowsersPerSuite trait" must {
    ...
    val jbrowser = try { val d = new JBrowserDriver(); d.quit(); 1 } catch { case ex: Throwable => 0 }
    val htmlUnit =
      try {
    ...
    val availableBrowserCount = chrome + firefox + internetExplorer + safari + htmlUnit + jbrowser
    ...
    "run all tests with different browsers available on the system one by one" in {
      ...
      assert(testStartingEventsReceived(10).testName == "test 1 [JBrowserDriver]")
      assert(testStartingEventsReceived(11).testName == "test 2 [JBrowserDriver]")
    ...
    "run only chosen test when ChosenTest tag is passed in" in {
      ...
      assert(testStartingEventsReceived(5).testName == "test 2 [JBrowserDriver]")
    ...
    "run only HtmlUnit and ChosenTest tests when HtmlUnit and ChosenTest tag is passed in" in {
      val expectedTestStartingCount = 8 // HtmlUnit * 2 + 4 other browsers on test 2, + test 3
      val expectedTestSucceededCount = (htmlUnit * 2) + firefox + chrome + internetExplorer + safari + jbrowser + 1
      val expectedTestCanceledCount = expectedTestStartingCount - expectedTestSucceededCount
      ...
      assert(testStartingEventsReceived(6).testName == "test 2 [JBrowserDriver]")
