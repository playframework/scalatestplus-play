...
object OneServerPerSuiteParallelSpec {
  var timesApplicationIsBuilt = new AtomicInteger(0)
  var timesServerIsStarted    = new AtomicInteger(0)
}
class OneServerPerSuiteParallelSpec extends UnitSpec with GuiceOneServerPerSuite with ParallelTestExecution {
  import OneServerPerSuiteParallelSpec._
  ...
  "The GuiceOneServerPerSuite trait, when running in parallel" must {
